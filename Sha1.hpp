...
namespace Sha1
{
	...
	std::string sha1UnsignedChar(const std::string &string)
	{
		std::string encrypted = sha1(string);
		...
		{
			std::stringstream ss;
			ss << std::hex << encrypted.substr(offset, 2);
			unsigned int strbuffer;
			ss >> strbuffer;
			hexCh += static_cast<unsigned char>(strbuffer);
			offset += 2;
		}
